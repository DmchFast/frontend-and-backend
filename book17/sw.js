...
self.addEventListener('fetch', event => {
    ...
    // Для динамических страниц (Network First)
    if (url.pathname.startsWith('/content/')) {
        event.respondWith(
            fetch(event.request)
                .then(networkRes => {
                    // Кэширование свежего ответа
    ...
    // Для всех остальных статик (Cache First)
    event.respondWith(
...
// Обработчик push‑событий (добавлена кнопка "Отложить")
